{-# LANGUAGE ConstraintKinds, ImpredicativeTypes, FlexibleContexts, GADTs #-}
...
editDistance (C i 0) = Just $ const $ pure i
editDistance (C 0 j) = Just $ const $ pure j
editDistance (C i j) = Just $ \fetch -> do
    ai <- fetch (A i)
    bj <- fetch (B j)
    if ai == bj
