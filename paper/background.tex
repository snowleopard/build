...
rebuild even if just a single task has changed. We further discuss
...
First, it uses the dependency from the previous build to detect which files need
to be rebuilt. This is a powerful idea with a long history of
use in \emph{incremental}~\cite{demers1981incremental},
...
\begin{itemize}
    ...
    \item \Excel stores a dirty bit per cell as well as the calculation chain.
...
$\!$\Shake      $\!$& Last dependency graph          & Recursive    & Dynamic      & Yes     & Yes    & No   $\!$\\
