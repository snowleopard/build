...
\todo{SLPJ}{Can we replace ``node'' with ``task''? Everywhere.}
\begin{enumerate}
...
In our model we define \hs{Task} as a function - when give the same inputs it will always produce the same outputs. Alas, the real-world is not as obliging. Some examples of impure \hs{Task} functions include:
