...
are also volatile. The main difference from non-deterministic tasks, is that
volatile tasks cannot be cached. They are best handled by introducing a special
key \cmd{RealWorld} whose value is changed between every run.
\end{description}
...
% (but its value not used) and \emph{also} baked directly into the \hs{sprsh4}
