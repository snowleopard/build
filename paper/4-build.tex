...
Section \S\ref{sec-background} introduced three different \emph{task scehedulers}
...
topological sort by storing the topological order between build runs,
using it as a starting point in the next run, 
fixing it up if the build discovers any inconsistency, as
\Excel does.
...
\noindent
Rather than storing (large) values \hs{v}, the verifying trace \hs{VT}
stores only hashes, of type \hs{Hash}~\hs{v}, of those values.  Since the
verfying trace persists from one build to the next -- it constitutes the build system's ``memory''
-- it is helpful for it to be of modest size.
After successfully building a key, we call \hs{recordVT} to add a record
to the current \hs{VT}, passing the key, the hash of
its value, and the list of hashes and dependencies.
...
terminal \emph{input keys}, but the technique works equally well
if we skip any number dependency levels (say $n$ levels).
The input-only approach is the special
case of $n = \infty$, and constructive trances are the special case of 
$n = 1$.
...
\simon{The data type decl had three args, k, v, r.}
...
With the information in this section we can build Table~\ref{tab-build-systems}, which
tabulates combinations of the schedling strategy and the out-of-date keys strategy,
providing 12~possible build systems, 8~of which are actually inhabited by existing build systems
...
many benefits, and which we title \Cloud \Shake
