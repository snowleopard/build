...
\noindent
As one can see, \hs{busy} correctly built both \cmd{B1} and \cmd{B2}, and in the
...
dependencies, which we address in the next subsection~\S\ref{sec-task-monad}.

\subsection{Monadic tasks}\label{sec-task-monad}

As discussed in~\S\ref{sec-background-excel}, some build tasks have dynamic
...
\todo{AM}{Describe another simple build system that does topological sort and
builds everything in order. It will not be able to handle dynamic dependencies.}
