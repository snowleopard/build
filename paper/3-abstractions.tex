...
systems. In addition to usual \hs{getValue} and \hs{putValue} operations, some
stores also support a dedicated \hs{getHash} operation, which is implemented
more efficiently than by hashing the result of \hs{getValue}. For example, GVFS
...
type \hs{i}, representing the persistent state maintained by the build system
itself~--~its~``memory''. We can \hs{initialise} a store by supplying an initial
value \hs{i} and a key-value map \store.
...
but not the input values.  Like every \hs{Task}, \hs{sprsh1} is given
...
our framework, such build tasks correspond to \emph{monadic task}, i.e.
the type \hs{Task}~\hs{Monad}~\hs{k}~\hs{v}. Recall the following spreadsheet
example:
