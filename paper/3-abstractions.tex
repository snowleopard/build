...
\noindent
The keys that belong to the transitive closure of \hs{key}'s dependencies fall
...
\begin{itemize}
    \item \hs{store} and \hs{result} \emph{agree on inputs}, that is, for all
    input keys \hs{k}~$\in$~$I$:
    \[
    \hs{getValue}~\hs{k}~\hs{store}~\hs{==}~\hs{getValue}~\hs{k}~\hs{result}.
    \]
    In other words, no inputs were corrupted during the
    build\footnote{\todo{SLPJ}{Add the joke that GHC used to delete inputs?}}.
    ...
    \[
    \hs{Just}~\hs{(}\hs{getValue}~\hs{result}~\hs{k)}~\hs{==}~\hs{compute}~\hs{task}~\hs{result}~\hs{k}.
    \]
\end{itemize}
...
\noindent
\todo{AM}{Type of track has changed - we no longer supply the Store, just an access fuction}
\todo{AM}{trackM is essential for Shake, what do we do there?}
