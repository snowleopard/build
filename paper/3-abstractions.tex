...
\paragraph{Persistent build information.}
The store also holds a value of type \hs{i},
...
\noindent
The \hs{busy} build system uses a helper function
\hs{compute} that, given a key, brings the key up to date in the store, and returns its value.
The function \hs{compute} runs in the standard Haskell \hs{State} monad -- see
Figure~\ref{fig-types} -- initialised with the incoming \hs{store} by \hs{execState}.
To bring a key up to date, \hs{compute} asks the task description \hs{task} how
to compute \hs{k}.
If \hs{task} returns \hs{Nothing} the key is an input, so \hs{compute} simply reads
the result from the store.
Otherwise \hs{compute} runs the action \hs{act}
returned by the \hs{task} to produce a resulting value~\hs{v}, records
the new key/value mapping in the store, and returns \hs{v}.
Notice that \hs{compute} passes itself to \hs{task} as an argument,
so that the latter can use \hs{compute} to recursively find the
values of \hs{k}'s dependencies.
...
\subsection{The need for polymorphism in \hs{Task}}

Now we can see why the \hs{Task} abstraction is polymorphic in \hs{f}:
\begin{minted}[xleftmargin=10pt]{haskell}
type Task c k v = @\std{forall}@ f. c f => (k -> f v) -> k -> Maybe (f v)
\end{minted}
The \hs{busy} build system instantiates \hs{f} to \hs{State (Store i k v)},
so that the call-back to \hs{fetch :: k -> f v} can side-effect the \hs{Store},
thereby allowing successive calls to \hs{fetch} to communicate with one another.
...
As we shall see, each build system will use a different \hs{f}, so the task description
must not fix \hs{f}.

But nor can the task description work for \emph{any} \hs{f}; most
task descriptions (e.g. \hs{sprsh1} in \S\ref{sec-task}) require that
\hs{f} satisfies certain properties, such as \hs{Monad} or
\hs{Applicatove}.  That is why \hs{task} has the ``\hs{c f =>}''
constraint in its type, expressing that \hs{f} can only be instantiated by types
that satisfy the constraints \hs{c}.
...
\begin{itemize}
\item \hs{Task Applicative}. In \hs{sprsh1} we needed only \hs{Applicative} operations,
...
\item \hs{Task Monad}.  As we shall see in \S\ref{sec-task-monad}, a monadic task
...
\item \hs{Task Functor} is somehat degenerate: the task description cannot even use
  the application operator \hs{<*>}, which limits dependencies to a single linear chain.
\end{itemize}

Notice also that, even though \hs{busy} takes a \hs{Task Monad} as its argument, an application of
\hs{busy} to a \hs{Task Applicative} will typecheck just fine.  It feels a bit
...
\hs{Task Monad k v}. Consider this spreadsheet
example:
...
then \hs{fetch} different keys depending on whether or not \hs{c1}~\hs{==}~\hs{1}}.
...
To express this formally we need an auxiliary
function \hs{recompute}, that computes the value of a key in a given store
...
we can use the standard Haskell \hs{Identity} monad.
The use of \hs{Identity} just fixes the `impedance mismatch'
...
\begin{itemize}
    ...
    \vspace{-1mm}
    \[
    \hs{recompute}~\hs{task}~\hs{ideal}~\hs{k}~\hs{==}~\hs{Just}~\hs{(}\hs{getValue}~\hs{ideal}~\hs{k)}.
    \]
...
\todo{SLPJ}{Please edit as you see fit. We need \hs{dependencies} and
\hs{track} to be defined for future sections. We also promised to define
\hs{inputs} in the previous section, but I think it's not too critical and can
be skipped/moved to Appendix if need be.}

Earlier we remarked that a \hs{Task Applicative} could only have static dependencies.
...
\hs{Task Applicative} to find its dependencies \emph{without dong any of the actual work}.
Here is the code:
\vspace{1mm}
\begin{minted}[xleftmargin=10pt]{haskell}
