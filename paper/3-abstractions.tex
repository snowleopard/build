...
Now we can see why the \hs{Task} abstraction needed to be abstracted over \hs{f}:
\begin{minted}[xleftmargin=10pt]{haskell}
type Task c k v = @\std{forall}@ f. c f => (k -> f v) -> k -> Maybe (f v)
\end{minted}
The \hs{busy} build system instantiates \hs{c} to \hs{Monad} and \hs{f} to \hs{State (Store i k v)},
...
We will see why we want \hs{Task} to be \emph{polymorphic} in \hs{f} in \S\ref{sec-deps}.
...
dependencies, which we discuss next (\S\ref{sec-task-monad}).
...
\noindent
We can use this function to extract dependencies of \hs{sprsh1}:
...
To extract the dependencies of monadic tasks we introduce the function \hs{track}:
