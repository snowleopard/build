...
\item \CloudBuild~\cite{esfahani2016cloudbuild} uses constructive traces with a topological sort.

\item \Buck~\cite{buck} uses deep constructive traces with a topological sort,
meaning that it disallows non-determinism~\S\ref{sec-non-determinism}.
...
combined with \cmd{import-from-derivation}, \Nix provides monadic dependencies.
\Nix is not primarily intended as a build system, and the coarse grained nature
