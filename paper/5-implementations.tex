...
\hs{modTime}~\hs{::}~\hs{k}~\hs{->}~\hs{Time} and the current time \hs{now}.
...
The helper function \hs{topological}, which we describe later, calls
...
\hs{key} needs to be rebuilt, the associated action \hs{act} is executed and
the obtained result is stored, along with an updated file modification
timestamp. The \hs{make} build system, as described, captures the essence of
