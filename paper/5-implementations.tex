...
the \hs{i} part of the \hs{Store}~\hs{i}~\hs{k}~\hs{v}. The \hs{newTask} can
find values of the \hs{key}'s dependencies via the \hs{fetch} callback that
reads the store by executing \hs{getValue} in the inner state monad.
...
framework as a combination of the \hs{restarting} scheduler and an
\hs{approximateRebuilder}.

\Excel's persistently stored information is a triple: (i) the set of dirty keys
\hs{Set}~\hs{k}, (ii) a map storing known upper bounds of dependencies
\hs{Map}~\hs{k}~\hs{[@@k]} that \Excel uses to handle dynamic dependencies, and
