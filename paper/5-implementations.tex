...
type Scheduler c i k v = Rebuilder c i k v -> Build c k v
type Rebuilder c i k v = k -> v -> Task c k v -> Task (MonadState i) k v
\end{minted}
That is:
\begin{itemize}
\item A \hs{Scheduler} is just a function that takes a \hs{Rebuilder} and uses
  it to construct \hs{Build} system, by choosing which keys to rebuild in which order.
\item A \hs{Rebuilder} is a function that is given a key, the key's ``old'' value,
