...
\noindent
A \hs{Scheduler} is just a function that takes a \hs{Rebuilder} and uses
...
The scheduler calls the rebuilder, but passes to it a \hs{fetch} function
...
\simon{topological is hard to read because of the execState and forM stuff.
  Could we have a local defn, with a type signature, for the $\lambda key. blah$?
  If necessary we could omit the code for liftStore, leaving only its signature}
...
ctRebuilder key value task = Task $ \fetch -> do
    ...
    if value `elem` cachedValues then return value else case cachedValues of
        ...
        _ -> do (newValue, deps) <- track task fetch
...
\simon{New subsection here? Or at least put this para after the next one.}
...
\hs{suspending} scheduler and is monadic. Deep constructive traces allow these
build systems to efficiently support shallow builds: the value of the target key
can be downloaded by examining only the hashes of all transitive inputs,
skipping the intermediate keys. The downside of this approach is the loss of the
