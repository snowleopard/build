...
calc \hs{Chain}~\hs{::}~\hs{[@@k]} recorded in the previous build
...
current \hs{value} as is. The external system, which invokes \Excel's build
engine, is required to provide a transitively closed set of dirty bits.
Therefore, unlike \Make's \hs{modTimeRebuilder}, the \hs{dirtyBitRebuilder} does
...
need the \hs{liftChain} helper function, whose implementation we omit since it
is analogous to \hs{liftStore} defined in Fig.~\ref{fig-make-implementation}.
