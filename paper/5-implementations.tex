...
We write a scheduler for each column of Table~\ref{tab-build-systems}, and a
rebuilder for each row; then applying the former to the latter, in any combination,
gives a correct build system.
...
of the scheduler-rebuilder decomposition in Table~\ref{tab-build-systems}. We discuss
each component in turn, starting with the rebuilder.

The rebuilder \hs{modTimeRebuilder} uses the pair
