...
\todo{NM}{We haven't used the term `target' before, perhaps change to `keys'?}
...
\todo{NM}{I found terms `predicate' and `generative' hard to grasp and a bit abstract. Especially `predicate'.}

\todo{NM}{I would also like to see an actual \hs{upToDate} function for predicate, and a \hs{recover}/\hs{fetch} or something like that for generative.}
