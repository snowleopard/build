...
not known statically\footnote{One might say that the value of \cmd{C1} is statically
...
requests to build \cmd{main.exe}. By looking up the history of all previous
builds, the build system finds that someone has already compiled these sources
before and obtains the hashes (4~and~5) of the resulting \cmd{util.o} and
\cmd{main.o} files (the files themselves do not need to be downloaded).
...
Fig.~\ref{fig-bazel}(c), the user modifies \cmd{util.c}, thereby changing its
...
\cmd{util.o}, which results in changing its hash from~4~to~8. The obtained
combination of hashes of \cmd{util.o} and \cmd{main.o} has not been encountered
before, therefore the build system first downloads \cmd{main.o} from the cache
and then builds \cmd{main.exe} by linking the two object files. When the build
