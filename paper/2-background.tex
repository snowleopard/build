...
\emph{calc chain}.  During recalculation, \Excel{} processes
cells in the calc-chain sequence, but if computing a cell C requires the
...
starting point so that, in the common case where changing an input
value does not change dependencies, there are no aborts.  Notice
