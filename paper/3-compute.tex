...
sprsh1 get "B1" = Just ((+) <$> get "A1" <*> "A2")
sprsh1 get "B2" = Just ((* 2) <$> get "B1")
sprsh1 _   _    = Nothing
\end{minted}
\vspace{1mm}
...
a \hs{get} function and a key. It pattern-matches on the key to see if it has
...
the value of the formula, using \hs{get} to find the value of any keys on
which it depends.
...
a side-effect-free way, using a call-back (\hs{get}) to get the values
of its dependencies.
