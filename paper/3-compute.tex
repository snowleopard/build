...
Equipped with the \hs{Compute} abstraction, we can now introduce \hs{Build} in
...
\noindent
The type closely matches the above definition: \hs{Compute c k v} is `a
description of tasks', a key~\hs{k} is~`a~specified output', and the store
\store holds `supplied inputs'. The additional input parameter \hs{Maybe i}
may provide auxiliary information from the previous build, as discussed
in~\S\ref{sec-background-summary}. The build system produces auxiliary
information \hs{i} for the next build as well as an updated store, where the
output \hs{k} must be up-to-date. We formally capture the correctness of the
resulting store in the following definition.

\definition[Correctness]{Let \hs{build :: Build c i k v} be a build system,
\hs{compute} describe an acyclic collection of build tasks, \hs{key} be an output
key, \hs{mi :: Maybe i} be auxiliary information, and \hs{store} be a store.
Then the \hs{result}ing store \hs{(_, result) = build compute k mi store}
is~\emph{correct} if there exists an \hs{ideal} store, such that

\begin{itemize}
    \item The \hs{ideal} store is \emph{consistent} with the \hs{compute}, i.e.
