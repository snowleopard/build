...
applicative functor \hs{Const} defined in GHC's standard library module
...
value of type \hs{Maybe}~\hs{(Const}~\hs{[}\hs{k]}~\hs{v)}, which we can unwrap
using \hs{maybe [@@] getConst}, i.e. returning the empty list of dependencies
in case of an input.

We can also execute a compute on a given store. This relies on a similar
transformation, now based on the \hs{Identity} monad defined in
\cmd{Data.Functor.Identity}:
...
the \hs{INDIRECT} function can be expressed as well: simply replace the last
line of the \hs{select} function with \hs{fetch ("A" ++ show c1)}.}:
...
% Note that this presentation of build tasks makes dynamic dependencies
% immediately recognisable: we need to examine the actual value \hs{c1} by
% extracting it from the computational context using \hs{Monad}'s bind statement
% \hs{c1 <- fetch}
