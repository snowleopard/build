...
cannot be computed, or \hs{Just value} obtained by \hs{fetch}ing the
...
power\footnote{Readers familiar with \emph{lenses} might recognise a familiar
pattern. We discuss this in~\S\ref{sec-related}.} by specifying the
constraint~\hs{c}. Let us look at an example by recalling the following
spreadsheet from~\S\ref{sec-background-excel}:
...
\noindent
We can express it as an \emph{applicative compute} as follows:
\vspace{1mm}
\begin{minted}[xleftmargin=10pt]{haskell}
...
\noindent
Cells \cmd{A1} and \cmd{A2} are inputs and \hs{add} returns \hs{Nothing} to
indicate this. The only task is associated with the cell \cmd{B1}: fetch the
values of \cmd{A1} and \cmd{A2} and apply the pure function~\hs{(+)} to them
using the methods of \hs{Applicative} type class.

%\hs{($) :: (a -> b) -> f a -> f b} and \hs{(<*>) :: f (a -> b) -> f a -> f b}
