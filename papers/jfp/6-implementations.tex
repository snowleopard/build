...
which tabulates 12 possible combinations, 8~of which are inhabited
by existing build systems (we discuss these systems in~\S\ref{sec-background}
and~\S\ref{sec-related-build}). Of the remaining 4~spots, all result in workable
build systems. The most interesting unfilled spot in the table corresponds to
a suspending scheduler composed with a constructive trace rebuilder. Such a
build system would provide many benefits; we title it \Cloud \Shake and explore
further in~\S\ref{sec-implementation-cloud}.

\vspace{-1mm}
\subsection{Concrete Implementations} \label{sec-concrete-impl}

We can define schedulers and rebuilders more concretely with the following types
(Fig.~\ref{fig-types}):
...
The scheduler runs the \hs{Task} returned by the rebuilder passing it a
\hs{fetch}~callback~that the task uses to find values of its dependencies.
The callback returns control to \mbox{the scheduler}, which may in turn call
the rebuilder again to bring another key up to date, and so on.
...
These two abstractions are the key to modularity: \emph{we can compose any
...
Table~\ref{tab-build-systems}, and a rebuilder for each row; then compose them
...
type~\hs{Build}~\hs{Applicative}. Any other build system that uses
the \hs{topological} scheduler will inherit the same restriction.
...
After the \hs{newTask} is executed (using \hs{liftStore}), there are two cases
to consider:
...
function and cannot be executed with the partial callback \hs{fetch}. We fix the
mismatch with the function \hs{try} that relies on the standard monad
transformer \hs{ExceptT}~\cite{liang1995monad}. The helper \hs{liftChain} is
analogous to \hs{liftStore} in Fig.~\ref{fig-make-implementation}, so we omit
its implementation.
...
As discussed in \S\ref{sec-step-traces}, \Shake actually uses verifying step
traces, but here we choose to focus on the more explicit verifying traces. We
have implemented verifying step traces in our framework, and they compose with
schedulers as you would hope.
...
queue}. The build starts with the queue containing all dirty keys. Similarly to
...
with constructive traces~\cite{esfahani2016cloudbuild}. We convert a
monadic \hs{ctRebuilder} into an applicative one by applying an adapter
\hs{adaptRebuilder}, which unwraps a given \hs{Task}~\hs{Applicative}
and wraps it into \hs{Task}~\hs{Monad}.
...
of \hs{ctRebuilder}. \Buck uses the \hs{topological} scheduler and is an
applicative build system, whereas \Nix uses the \hs{suspending} scheduler and
is therefore monadic.

% \todo{AM}{Add implementation of \hs{dctRebuilder}?}

Using the abstractions built thus far, we have shown how to compose schedulers
with rebuilders to reproduce existing build systems. To us, the most interesting
build system as yet unavailable would compose a suspending scheduler with
...
we need to do is compose \hs{suspending} with \hs{ctRebuilder},
