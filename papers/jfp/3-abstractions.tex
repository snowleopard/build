...
\hs{Monad} and \hs{MonadState}~\hs{s}.
...
lookup and store computation results in a cache by using the \hs{MonadState}
methods \hs{get} and \hs{set}. For example, below we implement a cloud version
of the Ackermann task that uses a \hs{Cache} of type
