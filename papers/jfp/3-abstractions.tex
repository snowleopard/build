...
newtype Task  c k v = Task
    { run :: @\std{forall}@ f. c f => (k -> f v) -> f v }
type    Tasks c k v = k -> Maybe (Task c k v)
\end{minted}
...
sprsh1 "B1" = Just $ Task $ \fetch ->
    ((+)  <$> fetch "A1" <*> fetch "A2")
sprsh1 "B2" = Just $ Task $ \fetch ->
    ((*2) <$> fetch "B1")
sprsh1 _    = Nothing
\end{minted}
\vspace{1mm}
...
compute task store = runIdentity $ run task $
    \k -> Identity $ getValue k store
\end{minted}
...
monad: the function
\hs{\@@k}~\hs{->}~\hs{Identity}~\hs{\$}~\hs{getValue}~\hs{k}~\hs{store} has the type
