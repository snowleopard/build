...
task is given a \emph{callback} \hs{fetch} to find the value of any keys on
which it depends. To run a \hs{Task}, we simply apply the function it holds to a
suitable callback (see the definition of the function \hs{run} in
Fig.~\ref{fig-types}).
...
\noindent
The \hs{busy} build system defines the callback \hs{fetch} that, when given a
key, brings the key up to date in the store, and returns its value.
The function \hs{fetch} runs in the standard~Haskell \hs{State} monad
(Fig.~\ref{fig-stdlib}) initialised with the incoming \hs{store} by \hs{execState}.
To bring a key~\hs{k} up to date, \hs{fetch} asks the task description \hs{tasks}
how to compute its value. If \hs{tasks} returns \hs{Nothing} the key is an
...
passes itself to \hs{task} as an argument, so the latter can use \hs{fetch}
...
correct order (if it had built \cmd{B2} before building \cmd{B1}, the result
would have been $20 * 2 = 40$ instead of $(10 + 20) * 2 = 60$). As an example
showing that \hs{busy} is not minimal, imagine that the formula in cell \cmd{B2}
was \cmd{B1~+~B1} instead of \cmd{B1~*~2}. This would lead to calling
\hs{fetch}~\hs{"B1"} twice -- once per occurrence of \cmd{B1} in the
formula -- and each call would recompute the formula in \cmd{B1}.

To avoid the recomputation, \hs{busy} can keep the set of processed keys in the
state monad (in addition to the \hs{store}), treat processed keys as inputs in
the \hs{getValue} branch of the \hs{fetch} callback, and include the key~\hs{k}
into the set of processed keys in the \hs{putValue} branch. This eliminates
unnecessary work \emph{within a single build}, but the next build needs to
recursively recompute all target's dependencies again even if no inputs changed.
To save work \emph{between builds}, it is necessary to store some build
information~\hs{i} persistently.
...
We really, really want \hs{Task} to be \emph{polymorphic} in \hs{f}. Given
\emph{one} task description \cmd{T}, we want to explore \emph{many} build
...
\item \hs{Task}~\hs{Monad}: As we shall see in \S\ref{sec-task-monad}, a monadic
  ...
  \cmd{D}. A simple example of a task with dynamic dependencies is \Excel
  formula \cmd{INDIRECT("A" \& C1)} from~\S\ref{sec-background-excel}.
...
\item \hs{Task}~\hs{Selective} corresponds to task descriptions with
  \emph{conditional statements}, e.g. \Excel formula \cmd{IF(C1=1,B2,A2)},
  where it is possible to statically \emph{over-approximate} the set of task
  dependencies. Here \hs{Selective} is a type class of \emph{selective
  applicative functors}~\cite{mokhov_selective_2019}, which allows us to model
  build systems like \Dune~\cite{dune} using the presented framework.
\item \hs{Task}~\hs{MonadFail} corresponds to monadic tasks that may fail. For
  example, the formula \cmd{A1/A2} may fail due to division by zero. We will
  discuss this in~\S\ref{sec-failures}.
\item \hs{Task}~\hs{MonadPlus}, \hs{Task}~\hs{MonadRandom} and their
...
\hs{fetch} \emph{different keys} depending on whether or not
\hs{c1}~\hs{==}~\hs{1}. Note that in this example one can statically determine
the sets of possible dependencies of the formulae \cmd{IF(C1=1,B2,A2)} and
\cmd{IF(C1=1,A1,B1)} but this cannot be done in general -- recall the
spreadsheet with the formula \cmd{INDIRECT("A" \& C1)}
from~\S\ref{sec-background-excel}, where the argument of the \cmd{INDIRECT}
function is a string computed dynamically during the build. Such tasks can also
be captured using \hs{Tasks}~\hs{Monad}:
...
This is another use of polymorphism in \hs{f}, discussed in~\S\ref{sec-why-polymorphism}.
...
The presented definition of correctness needs to be adjusted for build systems
that support non-deterministic tasks and shallow cloud builds, as will be
discussed in sections~\S\ref{sec-non-determinism} and~\S\ref{sec-cloud-aspects},
respectively.
...
has no such syntax tree: as you can see in the definition of \hs{Task} in
Fig.~\ref{fig-types}, a task is just a function, so all we can do is call it.
...
Here is an implementation of \hs{track} based on the standard Haskell
\hs{WriterT} \emph{monad transformer}~\cite{liang1995monad}, whose main
types are listed in Fig.~\ref{fig-stdlib}:
...
\noindent
This function uses the \hs{WriterT} transformer for recording additional
...
with dynamic dependencies, see~\S\ref{sec-implementation-shake}.

Here we show an example of \hs{track}ing monadic tasks when
\hs{m}~\hs{=}~\hs{IO}, by defining a corresponding \hs{fetchIO} of type
\hs{String}~\hs{->}~\hs{IO}~\hs{Integer}, which allows us to demonstrate the
dynamic nature of monadic dependencies in GHCi.
...
computed, e.g. see~Kosara~\shortcite{ackermann42}. We will use \hs{MonadState}
tasks in our models of cloud build systems in~\S\ref{sec-implementations}.
