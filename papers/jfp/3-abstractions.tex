...
\hs{Task} to every non-input key; input keys are associated with \hs{Nothing}.
The highly-abstracted type \hs{Task} describes how to build a value when given
...
The previous example shows why the \hs{Task} abstraction is polymorphic in
\hs{f}, recall the definition:
...
result has type \hs{Identity}~\hs{v}, which we unwrap with \hs{runIdentity}.

% To fix the mismatch, we unwrap using the function \hs{runIdentity} of type
% \hs{Identity}~\hs{a}~\hs{->}~\hs{a},
% which both forces the monad to \hs{Identity} and strips it away.
...
\subsection{Examples of Tasks}\label{sub-task-examples}

The \hs{Task} abstraction can be used to express pure functions in a way that is
convenient for their memoization, as will be further discussed
in~\S\ref{sec-related-memo}. Below we give examples of tasks corresponding to
several well-known functions.

The \emph{Fibonacci sequence} $F_n = F_{n-1} + F_{n-2}$ is the one of the most
favourite examples in the functional programming community and so we start with
it:
...
Fibonacci sequence by picking 0 and 1 for $n = 0$ and $n = 1$, respectively.
Any minimal build system will compute the sequence with memoization, i.e.
without recomputing the same value twice.

% TODO: Add a figure with a dependency graph?
The dependencies of Fibonacci numbers are statically known, hence we can express
it using \hs{Tasks}~\hs{Applicative}, benefiting from static dependency
analysis:
...
since it needs to perform an intermediate recursive call to determine one of its
dependencies $A(m, n-1)$:
...
\noindent
Now that we have seen examples of applicative and monadic tasks, let us finish
with an example of a functorial task -- the \emph{Collatz sequence}, where
given an initial value $c_0$, we calculate the next value $c_n$ from $c_{n-1}$
either by dividing $c_{n-1}$ by 2 (if it is even) or multiplying it by 3 and
...
Note that functorial tasks allow us to statically extract even more precise
information about their dependencies than applicative tasks. Indeed, we
statically know that a \hs{Task}~\hs{Functor} has exactly one \hs{dependency}:
