...
The development of \Cloud \Shake allows GHC developers to take advantage of
...
single-threaded \cmd{configure}) are responsible for a significant amount of
...
\item \textbf{Irrelevant differences}: A common problem is that you do not get
      ...
      \cmd{gcc} on different paths (say \cmd{/usr/bin} and \cmd{/usr/local/bin})
...
\item \textbf{Insufficient produced files}: A build rule must declare all files
      ...
      and only ever depend on \cmd{Foo.hi}, but \emph{use} \cmd{Foo.o} after
...
\item \textbf{Insufficient dependencies}: While insufficient dependencies are
      ...
      If you omit a dependency on a file that changes rarely, each developer will
      have built it locally at least once, and more regularly if they ever do clean
