...
In some sense, this is the ``free'' version of \hs{Task Monad} \cite{free-applicatives} -- the two types form an isomorphism. The original paper uses concrete types \hs{Key} and \hs{Value}, we generalise these types to \hs{k} and \hs{v}, and also add \hs{r} so that \hs{Action k v} can be an instance of \hs{Monad}. We define that \hs{Monad} instance, and use it to write conversion functions to/from our \hs{Task} type:
