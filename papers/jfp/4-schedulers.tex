...
\noindent
As we see, the \cmd{main.exe} is started first as it is the required target. It
soon discovers a dependency on \cmd{util.o} and suspends to build \cmd{util.o},
...
\q{Andrey}{It's unclear how our \hs{busy} build system fits into the two above
approaches. It probably is a third approach (i.e. using stack), but it's
unsuitable for real build systems that call external processes. Shall we mention
this?}

\q{Andrey}{A follow-up to the above: our \Shake models use neither of the two
above approaches, which seems a bit unsatisfactory. Can we at least hint how
our models could be adapted to use the continuation-passing style? Can we write
\hs{busyCPS}?}
