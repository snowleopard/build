...
referred to as \emph{build rules}, and their dependencies in a simple textual
form. For example:
...
the sake of simplicity. In reality, there are multiple versions of \cmd{gcc} and
the actual binary used to compile and link files, as well as standard libraries
...
example one might say that the value of \cmd{C1} is statically known, but
imagine that it is the result of a long computation chain -- its value will only
