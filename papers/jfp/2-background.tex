...
executing\footnote{In this example we pretend \cmd{gcc} is a pure function for
  ...
  To account for this, the actual binary for \cmd{gcc} is often also listed as a
  dependency, along with any supporting binaries, or standard libraries (such as
  \cmd{stdio.h}), that are used by \cmd{gcc}.
} the command
...
\subsection{\Shake: Dynamic Dependencies without Remorse}
