...
\q{Andrey}{Can we say something like: ``The authors are not aware of a build
system that uses a dirty bit rebuilder, is minimal, and supports the early
cutoff optimisaion''? Perhaps, we can even make a claim, that such a build
system is impossible with a believable argument/proof sketch?}
...
\q{Andrey}{The "function for obtaining the post-build value of any key" is a bit
mysterious. It would be nice to add a clarification. Or could we simplify the
interface and only provide a function with type
\hs{k -> Hash v -> [(k, Hash v)] -> VT k v -> Bool}? Presumably, \hs{verifyVT}
with the current type signature could be built on top of it?}
