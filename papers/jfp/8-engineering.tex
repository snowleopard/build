...
abstraction needs to be adjusted to cope with such keys (the prefix \hs{T}
stands for ``typed''):
...
\hs{TTask} now uses a typed \hs{Fetch} callback (we define a separate type
synonym only for readability), and (ii) the type of \hs{TTasks} is now
