...
callback \hs{fetchA1A2}. For example, the task description \hs{sprsh1}
from~\S\ref{sec-task} will return \hs{Just}~\hs{(Just}~\hs{30)} for the task
\cmd{B1 = A1 + A2}, \hs{Just}~\hs{Nothing} for the task \cmd{B2 = B1 * 2}
because \hs{fetchA1A2} failed, and simply \hs{Nothing} for the input \cmd{A1}.
...
\noindent
With this approach we can define a \hs{MonadFail} build system that can handle
errors by aborting the build early.
