...
from \cmd{A.hi+A.o}. We can express this pattern more clearly with the extra type safety from
...
The only awkward aspect is that both \cmd{A.hi} and \cmd{A.o} must ask for exactly the same
\cmd{Files} key. If one \hs{File} key swapped the order of the list to \hs{Files} then it would likely
run \cmd{ghc} twice. To help construct a well-formed multiple-output build task
...
\noindent In our example the partition function on either of the output names \hs{"A.hi"} or \hs{"A.o"} would
return \hs{["A.hi", "A.o"]}.

With a suitable \hs{Partition} it is possible to create a mapping that resolves \hs{File}
keys automatically into the correct \hs{Files} key.
...
task (Files ["A.hi","A.o"]) = Just $ TaskT $ \fetch ->
    -- run ghc here
-- more rules on elementary subsets as required
\end{minted}
\vspace{1mm}
