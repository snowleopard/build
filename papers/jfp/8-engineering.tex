...
in~\hs{f}, and can therefore choose to execute a \hs{Task} in an~\hs{f} with
failures, the simplest example being \hs{f}~\hs{=}~\hs{Maybe}. Below we define
a callback that returns \hs{Just}~\hs{value} for keys \cmd{A1} and \cmd{A2} but
fails with \hs{Nothing} on all other keys:
...
marked volatile, a technique \Excel takes with the \cmd{INDIRECT} function,
...
recomputing the task, e.g. the set \cmd{$\{$A1 + 1, A1 + 2$\}$} in the above
example.

% \todo{AM}{Show implementation of \hs{computeND}.
% Neil says: not convinced it's that interesting.}.

Interestingly, \hs{Task}~\hs{MonadRandom} is powerful enough to express
...
locally and evict \cmd{main.prof} from the cloud storage, (c)~rebuild
\cmd{main.prof} (profiling is non-deterministic, hence a new hash value), then
build \cmd{report.txt} by downloading it from the matching deep constructive
trace in the cloud. The result is a Frankenbuild because \cmd{main.prof} and
\cmd{report.txt} are inconsistent. New and evicted cloud storage entries are
highlighted; file hashes are shown in circles.
\label{fig-frankenbuild}}
% \vspace{-4mm}
\end{figure}

As described in \S\ref{sec-using-cloud-shake}, non-determinism
...
on the hashes of terminal task inputs (in this case \cmd{main.exe}). The result
...
We choose not to deal with cyclic dependencies -- a choice that most build
...
the \emph{oracle} rule in Mitchell~\shortcite{mitchell2012shake}, and users
...
One awkward aspect is that both \cmd{A.hi} and \cmd{A.o} must ask for
...
In the above function we compute the elementary subset \hs{ks} of the given key
\hs{k}, find the index of \hs{k} within the subset (using \hs{elemIndex}), run
the \hs{fetch} to build every key in the subset, then extract out the value
corresponding to \hs{k} (using the indexing operation \hs{!!}).
...
explained in the above \Excel example \hs{sprsh6}. The function takes a parser
for scripts, of type \hs{s}~\hs{->}~\hs{Task}~\hs{Monad}~\hs{k}~\hs{v}, and a
description of \emph{how to build all scripts}, of type
\hs{Tasks}~\hs{Monad}~\hs{k}~\hs{s}. For \hs{sprsh6}, the latter would simply
