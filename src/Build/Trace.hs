...
record :: Hashable v => Store i k v -> k -> [k] -> VT k v -> VT k v
record store key deps (VT ts) = VT (t : ts)
  where
    t = Trace key [ (k, getHash k store) | k <- deps ] (getHash key store)

-- | Given a function to compute the hash of a key's currnet value,
-- a @key@, and a set of verifying traces, return 'True' if the @key@ is
-- up-to-date.
verify :: (Monad m, Eq k) => (k -> m (Hash v)) -> k -> VT k v -> m Bool
verify fetchHash key (VT ts) = anyM match ts
  where
