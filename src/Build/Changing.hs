...
changingM :: (v -> Maybe (Task Monad k v)) -> Tasks Monad (KeyM k) v
changingM _     (KeyTaskM _) = Nothing -- the Tasks must be an input stored on disk
changingM _     (InputM   _) = Nothing
...
changingA1 parse (KeyA1 _ (Node task children)) = case parse task of
    Nothing -> Nothing
...
changingA2 _     _   (KeyTaskA2 _) = Nothing
changingA2 _     _   (InputA2   _) = Nothing
