...
-- @i@ from the previous build (which can be missing if this is the first build),
...
type Build c k v i = Ord k => Compute c k v -> [k] -> Maybe i -> Map k v -> (i, Map k v)
...
purify def sbuild compute outputs _ = runMapStore (sbuild compute outputs) def
...
correct :: (Ord k, Eq v) => Build Monad k v i -> Compute Monad k v -> Bool
...
idempotent :: (Ord k, Eq v) => Build Monad k v i -> Compute Monad k v -> Bool
